(function() {
  ...
  d3.layout.force = function() {
    ...
    force.drag = function() {
      if (!drag)
        drag = d3.behavior.drag().on("dragstart", dragstart).on("drag", d3_layout_forceDrag).on("dragend", d3_layout_forceDragEnd);
  ...
  d3.layout.stack = function() {
    ...
    stack.order = function(x) {
      if (!arguments.length)
        return order;
      order = typeof x === "function"
        ? x
        : d3_layout_stackOrders[x];
      return stack;
    };
    ...
    stack.offset = function(x) {
      if (!arguments.length)
        return offset;
      offset = typeof x === "function"
        ? x
        : d3_layout_stackOffsets[x];
      return stack;
    };
  ...
  d3.layout.hierarchy = function() {
    ...
    function revalue(node, depth) {
      ...
      if (children && (n = children.length)) {
        ...
        v = +value.call(hierarchy, d3_layout_hierarchyInline
          ? node
          : node.data, depth) || 0;
      }
      if (value)
    ...
    function tree(d, i) {
      ...
      d3_layout_treeVisitAfter_parent(root, function(node, previousSibling) {
        node._tree = {
          ...
          change: 0,
          shift: 0,
          number: previousSibling
            ? previousSibling._tree.number + 1
            : 0
        };
      });
  ...
  function d3_layout_treeRight_parents(node) {
    var parents = node.parents,
      n;
    return parents && (n = parents.length)
      ? parents[n - 1]
      : node._tree.thread;
  }
  ...
  d3.layout.treemap = function() {
    ...
    function scale(children, k) {
      var i = -1,
        ...
        area;
      while (++i < n) {
        area = (child = children[i]).value * (k < 0
          ? 0
          : k);
        child.area = isNaN(area) || area <= 0
          ? 0
          : area;
      }
    }
    ...
    function stickify(node) {
      ...
      if (children && children.length) {
        ...
        while (child = remaining.pop()) {
          ...
          row.area += child.area;
          if (child.z != null) {
            position(row, child.z
              ? rect.dx
              : rect.dy, rect, !remaining.length);
            row.length = row.area = 0;
          }
        }
    ...
    treemap.padding = function(x) {
      ...
      function padFunction(node) {
        ...
        return p == null
          ...
          : d3_layout_treemapPad(node, typeof p === "number"
            ? [p, p, p, p]
            : p);
      }
